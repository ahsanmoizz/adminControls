...
router.put("/:planKey", async (req, res) => {
  ...
  const { name, max_transactions, max_messages, price_usd, duration, features } = req.body;

  try {
    await db.query(
      `UPDATE subscription_plans
       SET name = $1, max_transactions = $2, max_messages = $3,
           price_usd = $4, duration = $5
       WHERE plan_key = $6`,
      [name, max_transactions, max_messages, price_usd, duration, planKey]
    );
